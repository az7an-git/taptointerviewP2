...
import { ClipboardCheck, ArrowRight, ArrowLeft, Building2, Clock, PauseCircle, XCircle, AlertCircle } from "lucide-react";
...
export default function ScreeningQuestionsPage() {
    ...
    const initScreeningSession = async () => {
      ...
      try {
        ...
        if (response.status === "success" && response.data && typeof response.data === "object") {
          const dataObj = response.data as { screening_token?: string; screening_attempt_id?: string; questions?: any[] };

          if (dataObj.screening_token) {
            localStorage.setItem("screening_token", dataObj.screening_token);
          }
          if (dataObj.screening_attempt_id) {
            localStorage.setItem("screening_attempt_id", dataObj.screening_attempt_id);
          }

          const rawQuestions = dataObj.questions || [];
          ...
          const backendMsg = typeof response.data === "string" ? response.data : ((response.data as any)?.message || response.status);
          setError(backendMsg);
        }
      } catch (err: any) {
        console.error("Failed to start screening session", err);
        setError(getParticipantErrorMessage(err, "Failed to start screening session"));
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const handleSubmitQuestions = async () => {
    ...
    try {
      ...
      setError(getParticipantErrorMessage(err, "Failed to submit answers. Please try again."));
    } finally {
    ...
    <>
      ...
      {isLoading ? (
        ...
        <div className="py-12 text-center text-gray-400 text-sm font-medium">
          {error ? (
            <div className="my-2 p-6 sm:p-8 rounded-2xl bg-white/[0.03] backdrop-blur-xl border border-white/10 shadow-2xl flex flex-col items-center text-center space-y-4 max-w-md mx-auto min-w-0 transition-all duration-300">
              <div className="w-14 h-14 rounded-full bg-[#FF512F]/10 border border-[#FF512F]/20 flex items-center justify-center text-[#FF512F] shadow-lg shadow-[#FF512F]/10">
                <AlertCircle className="w-7 h-7" />
              </div>
              <div className="space-y-1.5">
                <h3 className="text-base sm:text-lg font-bold text-white tracking-tight">Status</h3>
              ...
              <div className="pt-2 w-full flex justify-center">
                <button
                  className="px-6 py-2.5 rounded-xl bg-white/10 hover:bg-white/15 border border-white/15 text-white font-semibold text-xs tracking-wider uppercase transition-all duration-200 flex items-center justify-center gap-2 shadow-lg hover:shadow-white/5 cursor-pointer transform hover:scale-[1.02] active:scale-[0.98]"
                  onClick={() => navigate(-1)}
                >
                  <ArrowLeft className="w-3.5 h-3.5 text-[#FF512F]" />
                  <span>Go Back</span>
                </button>
              </div>
            </div>
  ...
  return (
    <ParticipantTwoPanelLayout
      ...
      containerClassName="mt-6 md:mt-12 mb-auto"
      leftPanel={leftPanel}
